...
validiere<IConfig>(geladeneConfig, IOConfig, "Laden der Konfiguration", fold(
    ...
    (existingConfig: IConfig) => {
        ...
        const isUnderline = (str: string | undefined): boolean =>
            str === undefined
                ? false
        ...
        const trim = (e: string | undefined): string => (e || "").trim();
        ...
        const weeksConfig: IWeekConfig[] = tableBody.map((row: string, rowIndex: number): IWeekConfig => {
        ...
        const fullConfig: IConfig = {...dummyConfig, ...existingConfig, numberOfTheLastWeek, weeks: weeksConfig};
